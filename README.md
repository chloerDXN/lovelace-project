...
We plan to primarily use the "Cooking Recipes" dataset which is 2.5GB and in the form of HTML files. As described in the associated paper's [source](http://infolab.stanford.edu/~west1/from-cookies-to-cooks/), the dataset contains data from 14 distinct high-traffic recipe sites. As the websites are in English, we decided to focus our project on access logs from the US where the number of clicks would be more representative than in a non-English speaking country.
